...
def summarize_performance_continuous(output_folder, step, gen_model, gan_model, latent_dim, num_cat, num_continuous, num_samples=9):
    ...
    for contin_i in range(num_continuous):
        ...
        contin_info = [1.5]*num_continuous
...
def summarize_performance_categorical(output_folder, step, gen_model, gan_model, latent_dim, num_cat, num_continuous, num_samples=9):
    """
    ...
    latent_codes = generate_ordered_latent_codes("span_categorical",
                                cat_info, 
                                other_cat_fixed_val, 
                                target_cat, 
                                num_samples_per_cat, 
    ...
    for i in range(num_samples_per_continuous*num_cat):
        ...
        plt.axis('off')
        plt.imshow(X[i, :, :, :], cmap='gray_r')
    filename1 = os.path.join(output_folder, f'generated_plot_{step+1}_cat.png')
    fig.tight_layout()  
    fig.savefig(filename1)
    plt.close()
